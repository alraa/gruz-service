...
$(function(){
	var pos_slide;
	if (($.cookie("slide_position") * 1) > 0)
		pos_slide = (($.cookie("slide_position") * 1) - 1)
	else pos_slide = 1;
	$(".slider").cycle({
		fx: 'fade',
		speed: 1200,
		timeout: 3000,
		pager: '.nav',
		startingSlide: pos_slide
	});

	$(window).unload(function() {
		$.cookie("slide_position", $(".activeSlide").html());
	})

// $("#layout_l, #layout_r").css("height", $(document).height() + 20 + "px");

	$(".c1 i, .c1 span, .c2 span, .c3 .s1").css({
		"opacity": 0
	});
	$(".c1 span, .c2 span, .c3 .s1").css({
		"display": "block"
	});
	$(".c1 i").css({
		"display": "inline"
	...
	$(".c1").live("mouseenter", function() {
		$(this).find("i, span").stop().animate({
			"opacity": 1
		}, 300);
	...
	$(".c3").live("mouseenter", function() {
		$(this).find(".s1").stop().animate({
			"opacity": 1
		}, 300);
		$(this).find("p:not(.s1)").stop().animate({
			"opacity": 0
		}, 300);
	}).live("mouseleave", function() {
		$(this).find(".s1").stop().animate({
	...
	$(".c2").click(function() {
		$.fancybox.open({
			...
			afterLoad: function() {
				...
				var content = '<form class="contact_us"><h1>Оставить заявку</h1><p><label for="v1">Имя</label> <input type="text" class="text" name="v1" id="v1" autocomplete="off" /></p><p><label for="v2">E-mail или телефон*</label> <input type="text" class="text" name="v2" id="v2" autocomplete="off" /></p><p><label for="v3">Информация о заказе</label> <textarea name="v3" id="v3" cols="10" rows="4" autocomplete="off"></textarea></p><p><label>&nbsp;</label> <a href="#" class="sendButton" rel="nofollow">РћС‚РїСЂР°РІРёС‚СЊ</a></p></form>';
	...
	$(".contact_us .sendButton").live("click", function() {
		ertotal = 0;


		if ($('#v2').val() == "") {
			$(".contact_us").find("p:eq(1)").css('color', 'red');
			...
			$(".contact_us").find("p:eq(1)").css('color', '#666');
			$('#v2').css('background-color', '#fff');
		}


		if (ertotal == 0) {
			var dt = $(".contact_us").serialize();
			$.post("/ajax/contact.php", dt, function(data) {
				if (data == 1) {
					alert("Спасибо!\nПисьмо было успешно отправлено.\nМы обязательно с вами свяжемся.");
					$(".contact_us input, .contact_us textarea").val("");
					$.fancybox.close();
				} else {
					alert("Отправка не удалась. Повторите попытку.");
				}
			}, "html");
		}
	...
	/* РЈР±РѕСЂРєР° РјСѓСЃРѕСЂР° */

	$('.musor-item-input input').on('keyup', function(event) {
		$('.musor-item').removeClass('active');
		$(this).parents('.musor-item').addClass('active');
		var num = parseInt($(this).val());
		if (!$(this).val()) {
			var total = 0;
			num = 0;
		} else {
			var total = parseInt($(this).val()) * 60;
		};

		$('.musor-item input').val('');
		$(this).val(num);

		total = total + parseInt($('.musor-item-input select').val());
		$('.musor-kalkulate-itog span').text(total);
	});

	$('.musor-item-input select').on('click', function(event) {
		if (parseInt($('.musor-item.active input').val()) == 'NaN') {
			var total = 0;
		} else {
			var total = parseInt($('.musor-item.active input').val()) * 60;
		};
	...
	$('.musor-item.active input').trigger('keyup');
	...
	$('#calk-uslug, #time-begin, #time-end, #num_work').on('change', function(event) {
		calculate();
	});
	$('#num_work').on('keyup', function(event) {
		calculate();
	});

	function calculate() {
		var uslug = parseFloat($('#calk-uslug option:selected').val());
		var begin = parseFloat($('#time-begin option:selected').val());
		var end = parseFloat($('#time-end option:selected').val());
		var work = $('#num_work').val();
		if (work == '') {
			work = 0;
		} else {
			work = parseFloat(work);
		}
		var itog = (end - begin) * work * uslug;
		if ((end - begin) > 0) {
			$('.calk__price span').html(itog + ' руб.');
		} else {
			$('.calk__price span').html('0 руб.');
		}
	}

	$('#calk-uslug').html('');

	if ($('.calk').hasClass('calk--razn')) {
		...
		for (y in uslugiRazd) {
			...
			html = html + uslugiRazd[y]['price'] + '">' + uslugiRazd[y]['name'] + '</option>"'
		}
		...
		for (y in uslugi) {
			html = html + '<option value="';
			html = html + uslugi[y]['price'] + '">' + uslugi[y]['name'] + '</option>"'
		}
	}
	$('#calk-uslug').append(html);

	$('#calk-uslug, #time-begin, #time-end, #num_work').trigger('change');

	function culcAutoriz() {
		var worker = parseFloat($('#worker option:selected').val());
		var smena = parseFloat($('#smena option:selected').val());
		var number = $('#numbers').val();
		if (number == '') {
			number = 0;
		}
		var itog = worker * 8 * smena * number;
		$('.autsor-calk__itog span').html(itog + ' рублей');
	}

	$('#worker, #smena').on('change', function(event) {
		culcAutoriz();
	});
	...
	$('.stirka-button').on('click', function(event) {
		var error = false;
		if ($('#name').val == '') {
			$('#name').addClass('error');
			error = true;
		...
		if (!error) {
			var arr = new Array();
			var classes = $('.calk--online input, .calk--online select, .calk--online textarea').map(function(indx, element) {
				if ($(element).data('name')) {
					arr.push($(element).data('name') + ': ' + $(element).val());
					console.log($(element).data('name') + ': ' + $(element).val());
				};
			});
			$.ajax({
				type: "POST",
				...
				alert("Спасибо!\nПисьмо было успешно отправлено.\nМы обязательно с вами свяжемся.");
				$('.calk--online input, .calk--online select, .calk--online textarea').val('');
			});
		} else {
	...
	$('.upak-form__button').on('click', function(event) {
		var error = false;

		var arr = new Array();
		var classes = $('.upak-form input, .upak-form select').map(function(indx, element) {
			if ($(element).val() == '') {
	...
	$('.slate-before').on('click', function() {
		$(this).siblings('select').focus().click();
	});

	$('.razn__slide__form__button').on('click', function(event) {
		var error = false;

		var arr = new Array();
		var classes = $('.razn__slide__form input, .razn__slide__form textarea').map(function(indx, element) {
			if ($(element).val() == '') {
				$(element).addClass('error');
				error = true;
			} else {
		...
		if (!error) {
			$.ajax({
				...
				data: {
					dataform: arr,
					formname: 'Заказ разнорабочих на стройку'
				}
			}).done(function(msg) {
				alert("Спасибо!\nПисьмо было успешно отправлено.\nМы обязательно с вами свяжемся.");
			});
	...
	$('.sbor__zakaz .burron').on('click', function(event) {
		var error = false;

		var arr = new Array();
		var classes = $('.sbor__zakaz input, .sbor__zakaz select').map(function(indx, element) {
			if ($(element).val() == '') {
				$(element).addClass('error');
				error = true;
			} else {
	...
	$("#slider1").slider({
		...
		slide: function(event, ui) {
			$("#valslider1").val(ui.value);
		}
	});

	$("#valslider1").val($("#slider1").slider("value"));

	$("#valslider1").keyup(function() {
		$("#slider1").slider("value", $("#valslider1").val());
	});
	...
	$("#slider2").slider({
		...
		step: 1,
		slide: function(event, ui) {
			$("#valslider2").val(ui.value);
		}
	});

	$("#valslider2").val($("#slider2").slider("value"));

	$("#valslider2").keyup(function() {
		$("#slider2").slider("value", $("#valslider2").val());
	});


	$('.grp-calk__button').on('click', function(event) {
		var error = false;

		var arr = new Array();
		var classes = $('.grp-calk input, .grp-calk select').map(function(indx, element) {
			if ($(element).val() == '') {
				$(element).addClass('error');
				error = true;
...
function calkSnow() {
		var total;
		total = Number($('#metr').val()) * 250;

		total += Number($('#krol').val()) * Number($('#ruch').val());

		if(Number($('#mech').val()) == 1){
			total += 14000;
		} else if(Number($('#mech').val()) == 2) {
			total += 11000;
		}

		if(Number($('#tech').val()) == 1){
			total += 7000;
		}
		else if(Number($('#tech').val()) == 2) {
			total += 9400;
		} else if(Number($('#tech').val()) == 3) {
			total += 6000;
		}
		if($('#regime2').is(":checked")) {
			total = Math.ceil(total * 1.1);
		}

		$('.snow-calk__itog__text span').html(total + ' руб.');
	}


	$("#metr, #ruch, #tech").keyup(function() {
		calkSnow();
	});

	$('#metr, #krol, #ruch, #mech, #tech, #regime1, #regime2').on('change', function(event) {
		calkSnow();
	});
	...
	$('.rating__min__button').on('click', function() {
		if ($('.rating__min__form input').val() != '') {
			if (parseInt($('.rating__min__form input').val()) > 5) {
				alert('Оценка должна быть от 1 до 5');
			} else {
				var arr = new Array();
				arr.push('Рейтинг:' + $('.rating__min__form input').val());
				$.ajax({
					type: "POST",
					url: "/ajax/submit.php",
					data: {
						dataform: arr,
						formname: 'Рейтинг оценки качества'
					}
				}).done(function(msg) {
					alert("Спасибо!\nВаш голос учтен.");
				});
